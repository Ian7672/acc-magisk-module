...
# Global modification explanation
printf "\n---\n🔧 Global modification by github.com/Ian7672:\n"
printf "  ⚡ Stop charging: 90%%\n"
printf "  🔋 Resume charging: 80%%\n"
printf "  🛌 Idle mode: 90%%\n"
printf "  💀 Safe shutdown minimum: 2%%\n"
printf "  ❌ No forced shutdown when battery is hot\n"
printf "  🔔 Notifications:\n"
printf "     - 🔌 Charger connected\n"
printf "     - ⏸️ Charging paused at 90%%\n"
printf "     - ▶️ Charging resumed at 80%%\n"
printf "     - 🌡️ Battery warm (≥45°C)\n"
printf "     - ⚠️ Battery hot (≥55°C)\n"
printf "     - 🛑 Idle mode enabled\n"
printf "     - 📉 Battery low (<20%%, once per level, reset on unplug)\n"
printf "---\n\n"
...
[ -f $srcDir/README.md ] && sed -En "\|^## LINKS|,\$p" $srcDir/README.md \
  | grep -v '^---' | sed 's/^## //' || echo "README.md not found"

printf "\n\nCHANGELOG\n\n"
[ -f $srcDir/changelog.md ] && cat $srcDir/changelog.md || echo "changelog.md not found"
